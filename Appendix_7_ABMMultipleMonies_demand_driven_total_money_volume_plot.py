...
class MyModel(Model):
	...
	def __init__(self, NSuppliers, NDemanders, InitialMoney, prop_hel, prop_expand, prop_rec):
		...
		self.data_collector = DataCollector(
			model_reporters = {"Total Volume (Sum) of all Monies": return_total_money_volume})
...
for i in range(50):
	model.step()
total_money_volume = model.data_collector.get_model_vars_dataframe()
total_money_volume.plot()
plt.show()