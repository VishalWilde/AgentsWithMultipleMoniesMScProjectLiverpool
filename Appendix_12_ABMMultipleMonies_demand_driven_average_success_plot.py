...
batch_run = BatchRunner(MyModel, 
						...
						iterations=30, 
						max_steps=50, 
						model_reporters = {"Average_Success_Rate": return_average_success_rate, "Helicopter_Money_Success_Rate": return_helicopter_success_percentage})

batch_run.run_all()

run_data = batch_run.get_model_vars_dataframe()
run_data.head()
plt.scatter(run_data.NSuppliers, run_data.Average_Success_Rate, label = 'Average Success Rate')
plt.scatter(run_data.NSuppliers, run_data.Helicopter_Money_Success_Rate, color = 'r', marker = 'X', label = 'Helicopter Money Success Rate')
plt.xlabel("Number of Money Suppliers (tested in intervals of 3 to save time)")
plt.ylabel("Average Success Rate (%) / Helicopter Money Success Rate (%) over 50 time steps")
plt.title("The influence of the number of money-suppliers on success rate.")
plt.legend()
