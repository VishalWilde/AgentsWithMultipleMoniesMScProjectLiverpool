...
param_values = {"NSuppliers": 5, "NDemanders": range(5,31), "InitialMoney": 1000, "prop_hel": 10, "prop_expand": 10, "prop_rec": 50}

batch_run = BatchRunner(MyModel, 
						param_values, 
						iterations=50, 
						max_steps=1, 
						model_reporters = {"Money_Value_Growth_Rate": return_overall_model_money_value_growth})
batch_run.run_all()

run_data = batch_run.get_model_vars_dataframe()
run_data.head()
plt.scatter(run_data.NDemanders, run_data.Money_Value_Growth_Rate, color = 'r', marker = 'X', label = 'Money Value Growth Rate')
plt.xlabel("Number of Money Demanders")
plt.ylabel("Exchange-Weighted Money Value Growth Rate (%) after first time step")
plt.title("The influence of the number of money-demanders on the economy's/economies' growth rate(s)")
plt.legend()
