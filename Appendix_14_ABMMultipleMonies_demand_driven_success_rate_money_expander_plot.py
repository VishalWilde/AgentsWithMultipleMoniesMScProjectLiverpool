...
def return_helicopter_success_percentage(model):
	return model.helicopter_success_rate
	
def return_overall_model_money_value_growth(model):
	percentage_growth = (model.final_exchange_weighted_money_value / model.first_exchange_weighted_money_value)
	percentage_growth = (percentage_growth - 1)
	percentage_growth = (percentage_growth * 100)
	return percentage_growth

def return_average_success_rate(model):
	average_success_rate = (model.total_success_rate / model.model_step)
	return average_success_rate
	
def return_prop_expanders_of_successful(model):
	prop_expanders_of_succesful = model.expander_true / model.model_step
	return prop_expanders_of_succesful
...
class MyModel(Model):
	...
	def __init__(self, NSuppliers, NDemanders, InitialMoney, prop_hel, prop_expand, prop_rec):
		...
		self.prop_hel = (prop_hel/100) 
		self.prop_expand = (prop_expand/100) 
		self.prop_rec = (prop_rec/100)
		self.running = True
		...
		self.first_exchange_weighted_money_value = 0.0
		for i in range(self.num_suppliers):
			self.first_exchange_weighted_money_value += (self.market.default_exchange_rates[i] * self.market.volume_of_each_money[i])
		#self.data_collector = DataCollector(
		#	model_reporters = {"Absolute Social Welfare": return_total_social_welfare})
		
		self.total_success_rate = 0.0
		self.expander_true = 0.0
	...
	def step(self):
		...
		for x in self.percentage_changes:
			if x < 1 and x > -1:
				num_success += 1
				if self.percentage_changes_volumes[index] > 0:
					expander_num += 1
			index += 1		
		...
		if self.success == True:
			self.expander_true += (expander_num / num_success)
		...
		if self.Helicopter_Money == True:
			self.helicopter_success_rate = self.percentage_success
		
		self.total_success_rate += self.percentage_success			
...
#Batch running model to test the success rate of helicopter money with 
#only varying the proportion of agents that receive helicopter money (from 10% to 100% in
#intervals of 10%), proportion of helicopter money compared to the total money in the system 
#is kept at 10% whilst proportion of money expanders is kept at 10%. 
#It is run over 30 times (35 times precisely) so that Central Limit Theorem applies. 

fixed_params = {"NSuppliers": 10, "NDemanders": 10, "InitialMoney": 1000, "prop_hel": 10, "prop_expand": 10}

variable_params = {"prop_rec": range(10, 110, 10)}

param_values = {"NSuppliers": range(3,10,3), "NDemanders": 10, "InitialMoney": 1000, "prop_hel": 10, "prop_expand": 10, "prop_rec": 50}

batch_run = BatchRunner(MyModel, 
						param_values, 
						iterations=35, 
						max_steps=50, 
						model_reporters = {"proportion_of_expanders_of_successful": return_prop_expanders_of_successful})
batch_run.run_all()

run_data = batch_run.get_model_vars_dataframe()
run_data.head()
plt.scatter(run_data.NSuppliers, run_data.proportion_of_expanders_of_successful, color = 'r', marker = 'X', label = 'Percentage of fixed exchange rate monies with money-expansions')
plt.xlabel("Number of Money Suppliers")
plt.ylabel("Corresponding money-expansions alongside fixed exchange rates (%)")
plt.title("Investigating whether successfully fixed exchange rates accompany corresponding money-expansions")
plt.legend()
