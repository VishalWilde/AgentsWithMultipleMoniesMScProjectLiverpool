...
def return_overall_model_money_value_growth(model):
	percentage_growth = (model.final_total_money_volume / model.initial_money)
	percentage_growth = (percentage_growth - 1)
	percentage_growth = (percentage_growth * 100)
	return percentage_growth
	
def return_helicopter_prop_expander_success(model):
	return model.helicopter_prop_expander_success
...
class MyModel(Model):
	...
	def __init__(self, NSuppliers, NDemanders, InitialMoney, prop_hel, prop_expand, prop_rec):
		...
		self.helicopter_prop_expander_success = 0.0
	...
	def step(self):
		...
		if self.Helicopter_Money == True:
			...
			if self.success == True: 
				prop_expander_success = (expander_true / num_success)
				self.helicopter_prop_expander_success = (prop_expander_success * 100)
...
param_values = {"NSuppliers": 6, "NDemanders": 10, "InitialMoney": 1000, "prop_hel": 10, "prop_expand": 10, "prop_rec": range(10, 115, 15)}

batch_run = BatchRunner(MyModel, 
						param_values, 
						iterations=30, 
						max_steps=30, 
						model_reporters = {"Helicopter_Money_Success_Percentage": return_helicopter_success_percentage, 
						"Percentage_of_money_expanders": return_helicopter_prop_expander_success})

batch_run.run_all()

run_data = batch_run.get_model_vars_dataframe()
run_data.head()
plt.scatter(run_data.prop_rec, run_data.Helicopter_Money_Success_Percentage, label = 'Heicopter Success Percentage')
plt.scatter(run_data.prop_rec, run_data.Percentage_of_money_expanders, color = 'r', marker = 'X', label = 'Percentage of money expanders')
plt.xlabel("Percentage receiving Helicopter Money")
plt.ylabel("Helicopter Money Success (%) / corresponding money-expanders (%)")
plt.title("Influence of proportion receiving Helicopter Money on success rates")
plt.legend()
