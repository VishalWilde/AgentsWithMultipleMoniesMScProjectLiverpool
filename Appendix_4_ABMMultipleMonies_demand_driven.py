...
class MoneySupplier(Agent):
	...
	#creates money and adds it to the market whilst also updating the interest rates
	#and exchange rates and all the money suppliers' utility. 
	...
	#destroy money method that is never used in this program but could be used in 
	...
	#determine optimum money-supply proportion for the respective money-supplying agent
	#w.r.t to their own money (univariate optimisation problem - numerical approximation).
	def determine_opt_money_supply(self):
		...
		#initialises the temporary numpy arrays required for the following tests to 
		#determine the optimum money supply that the agent would like to see. 
...
class MoneyDemander(Agent):
	...
	def optimal_portfolio_quantities(self):
		...
		#determining the optimum portfolio quantities and the changes required in this
		#particular portfolio to achieve it. 
	...
	#for another - this goes through the entire list of monies demanded and is not 
	#invoked in this particular program but could be useful in future iterations of 
	#this ABM.
	...
	#same as above method but just for one money rather than multiple monies. This is used
	#in this program rather than the above (to save time).
	def minimise_money_paid(self, amount_demanded, money_index):
		...
		#setting up coefficient matrix and upper bound values for the optimisation problem.
	...
	def step(self):
		...
		for x in monies_demanded_with_utilities:
			...
			#if the money-supplier does not have the quantity of money required by the money-demander.
...
class MyModel(Model):
	...
	def __init__(self, NSuppliers, NDemanders, InitialMoney, prop_hel, prop_expand, prop_rec):
		...
		self.data_collector = DataCollector(
			model_reporters = {"Success Rate": return_success_rate})
	...
	def step(self):
		...
		#calculating the initial exchange weighted money value. 
		self.initial_exchange_weighted_money_value = 0.0
		for i in range(self.num_suppliers):
		...
		#deciding whether tests were succesful and calculating rates accordingly for 
...
for i in range(50):
	model.step()
success = model.data_collector.get_model_vars_dataframe()
success.plot()
plt.show()