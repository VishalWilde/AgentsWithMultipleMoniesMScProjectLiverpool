...
param_values = {"NSuppliers": range(3,15), "NDemanders": 10, "InitialMoney": 1000, "prop_hel": 10, "prop_expand": 10, "prop_rec": 50}

batch_run = BatchRunner(MyModel, 
						param_values, 
						iterations=50, 
						max_steps=1, 
						model_reporters = {"Overall_Money_Value_Growth_Percentage": return_overall_model_money_value_growth})

batch_run.run_all()

run_data = batch_run.get_model_vars_dataframe()
run_data.head()
plt.scatter(run_data.NSuppliers, run_data.Overall_Money_Value_Growth_Percentage)
plt.xlabel("Number of Money Suppliers")
plt.ylabel("Weighted Money Value Growth (%)")
plt.title("Weighted Money Value Growth (%) scatter plot after one time step")
plt.legend()
